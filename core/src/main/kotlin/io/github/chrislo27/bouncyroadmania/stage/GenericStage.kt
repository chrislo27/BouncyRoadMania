...
open class GenericStage<S : ToolboksScreen<*, *>>(override var palette: UIPalette, parent: UIElement<S>?, camera: OrthographicCamera)
    : Stage<S>(parent, camera), Palettable {

    companion object {
        const val SCREEN_WIDTH = 0.85f
        const val SCREEN_HEIGHT = 0.9f
        val AREA = (BRMania.WIDTH * SCREEN_WIDTH) to (BRMania.HEIGHT * SCREEN_HEIGHT)
    ...
    override fun render(screen: S, batch: SpriteBatch,
        ...
        val height = (8f / BRMania.WIDTH) * camera.viewportHeight
