    ...
    private fun openPicker() {
        if (!isChooserOpen) {
            GlobalScope.launch {
                ...
                val file = TinyFDWrapper.openFile(Localization["musicSelect.fileChooserTitle"], attemptRememberDirectory(PreferenceKeys.FILE_CHOOSER_EDITOR_MUSIC)?.absolutePath?.plus("/"), filter)
                isChooserOpen = false
                if (file != null) {
                    loadFile(file)
                }
